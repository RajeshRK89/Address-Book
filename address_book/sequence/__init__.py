...
def get_last_value(sequence_name="default", *, using=None):
    """
    ...
    with connection.cursor() as cursor:
        cursor.execute(SELECT.format(db_table=db_table), [sequence_name])
...
def get_next_value(sequence_name="default", initial_value=1, reset_value=None, *, nowait=False, using=None):
    ...
    if (
        ...
        with connection.cursor() as cursor:
            cursor.execute(POSTGRESQL_UPSERT.format(db_table=db_table), [sequence_name, initial_value])
            result = cursor.fetchone()
        ...
        with transaction.atomic(using=using, savepoint=False):
            sequence, created = Sequence.objects.select_for_update(nowait=nowait).get_or_create(
                name=sequence_name, defaults={"last": initial_value}
            )
...
class Sequence:
    ...
    def __init__(self, sequence_name="default", initial_value=1, reset_value=None, *, using=None):
        if reset_value is not None:
            assert initial_value < reset_value
